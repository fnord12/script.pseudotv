...
class TVOverlay(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        try:
            if self.AlwaysUseStartChannel == True:
                debug ('start channel should always be based on setting: ', self.fixChannel(int(self.StartChannel)))
                ...
                if self.forceReset == False and self.channelResetSetting is not "5":
                    ...
                    debug ('start channel should be based on setting due to channel reset: ', self.fixChannel(int(self.StartChannel)))
                    self.currentChannel = self.fixChannel(int(self.StartChannel))
        except:
    ...
    def showInfo(self, timer):
        ...
        debug("xbmc.getCondVisibility('Player.ShowInfo') = ", xbmc.getCondVisibility('Player.ShowInfo'))
    ...
    def fixChannel(self, channel, increasing = True):
        debug('fixchannel channel=', channel)
    ...
    def notificationAction(self):
        ...
        if self.Player.isPlaying():
            ...
            if docheck == True:
                ...
                #Don't show any notification if the current show is < 360 seconds
                #if self.channels[self.currentChannel - 1].getItemDuration(self.notificationLastShow) < 360:
                #        self.notificationShowedNotif = True
